...
class DatatableMixin(object):
    ...
    def filter_queryset(self, qs):
        """ If search['value'] is provided then filter all searchable columns using the search_condition defined
        """
        if not self.pre_camel_case_notation:
            ...
            for col_no, col in enumerate(col_data):
                filter_key = '{0}__{1}'.format(self.columns[col_no].replace('.', '__'), self.search_condition)

                # apply global search to all searchable columns
                if search and col['searchable']:
                    q |= Q(**{filter_key: search})

                # column specific filter
                if col['search.value']:
                    qs = qs.filter(**{filter_key: col['search.value']})
            qs = qs.filter(q)
        return qs
